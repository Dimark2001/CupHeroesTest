...
public class PlayerObj : MonoBehaviour, ICharacterAnimation
{
    ...
    public void PlayStateAnimation(PlayerState state){
        _ = _prefabs.PlayAnimation(state, IndexPair[state]);
    }
    void Update()
    {
        if(isAction) return;
    ...
    public async UniTask AttackAnimation(ICharacterPresenter target){
        isAction = true;
        await _prefabs.PlayAnimation(PlayerState.ATTACK, 0);
    }
    
    [Button]
    public async UniTask DamagedAnimation(){
        isAction = true;
        await _prefabs.PlayAnimation(PlayerState.DAMAGED, 0);
    }
    
    [Button]
    public async UniTask DeathAnimation(){
        isAction = true;
        await _prefabs.PlayAnimation(PlayerState.DEATH, 0);
    }

    [Button]
    public async UniTask MoveAnimation(){
        isAction = true;
        await _prefabs.PlayAnimation(PlayerState.MOVE, 0);
    }
    
    [Button]
    public async UniTask IdleAnimation(){
        isAction = true;
        await _prefabs.PlayAnimation(PlayerState.IDLE, 0);
    }
}